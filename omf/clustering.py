"""Logic for clustering features mined from Yelp reviews.
...
class FeatureMapper(object):
    def __init__(self, df_extractions: pd.DataFrame, feature_categories: list = None, feature_amenity_map: dict = None):
        ...
        self.categories = feature_categories if feature_categories else FEATURE_CATEGORIES
    ...
    def cluster_features(self):
        ...
        self._create_tdm()
        init_centroids = self.get_centroids()
        ...
        self.kmeans = KMeans(init=init_centroids, n_init=1, max_iter=1, n_clusters=len(init_centroids))
    ...
    def clusters_to_feature_vectors(self, category_features=None):
        ...
        # _clusters is a dictionary, where each key is an integer index corresponding to a particular cluster,
        # and each value is an OrderedDict whose items corresponds to features in the cluster and their distances.
        # So for each item in the OrderedDict, the key is the feature, and the value is the distance of the feature
        # to the centroid of the cluster.
        _clusters = self.clusters_to_dict(features=list(self.feature_map.values()))

        logger.info('Validating integrity of clusters...')
        for this_key, this_features in _clusters.items():
            for that_key, that_features in _clusters.items():
                if this_key != that_key:
                    duplicates = set(this_features.keys()).intersection(set(that_features.keys()))
                    if len(duplicates) != 0:
                        logger.info('Duplicates found in {} and {}: {}'.format(this_key, that_key, len(duplicates)))

        for idx, (cluster_idx, clustered_features) in enumerate(_clusters.items()):
            feature = self.centroid_feature_lookup[idx]
            feature_vectors[feature] = clustered_features

        # for feature in sorted(category_features):
        # for feature, category in self.feature_amenity_map.items():
    ...
    def get_centroids(self):
        ...
        feature_idx_lookup = {v: k for k, v in self.feature_map.items()}
        rows = list()

        self.centroid_feature_lookup = list()

        bad_features = []
        for feature, category in self.feature_amenity_map.items():
            if feature in feature_idx_lookup:
                feature_idx = feature_idx_lookup[feature]
                rows.extend(self.tdm[feature_idx].todense().tolist())
                self.centroid_feature_lookup.append(feature)
            else:
                # Features not in feature_map should be recorded and removed.
                bad_features.append(feature)

        for feature in bad_features:
            msg = 'Removing {} from feature_amenity_map because it is non-existent...'
            logger.info(msg.format(feature))
            del self.feature_amenity_map[feature]
